...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/19 15:20:14 by thibnguy          #+#    #+#             */
/*   Updated: 2022/12/20 18:12:16 by thibnguy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(const int fd)
{
	...
	char		*entire_line;
	static char	*row[4096];

	all_lines = NULL;
	if (read(fd, 0, 0) < 0 || BUFFER_SIZE <= 0)
		return (free(row[fd]), row[fd] = NULL);
	all_lines = get_all_lines(fd, buf, row[fd]);
	if (!all_lines)
		return (NULL);
	entire_line = get_entire_line(all_lines);
	row[fd] = get_rest(all_lines);
	free(all_lines);
	all_lines = NULL;
	return (entire_line);
}
